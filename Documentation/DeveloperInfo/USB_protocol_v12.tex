...
32 & StopAutoIdle & H$\rightarrow$D & Stops automatic VNA idling after timeout & None\\
33 & StartAutoIdle & H$\rightarrow$D & Starts automatic VNA idling after timeout & None\\
\end{longtable}   
\end{ThreePartTable}
...
\subsection{StopAutoIdle}
This packet instructs the device to stop automatically going to idle mode after fixed timeout of 100ms when buffer is full (i.e. when VNADatapoint packets are not being actively read from device).

\subsection{StartAutoIdle}
This packet instructs the device to resume automatically going to idle mode after fixed timeout of 100ms when buffer is full (i.e. when VNADatapoint packets are not being actively read from device). This restores the default behavious if a StopAutoIdle packet has previously been sent.
